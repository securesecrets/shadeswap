...
pub fn amm_pair_integration_tests() {    
    ...
    use multi_test::help_lib::integration_help_lib::{roll_blockchain, mint_deposit_snip20, send_snip20_to_stake, snip20_send, increase_allowance, get_current_block_time, store_init_staking_contract, store_init_factory_contract, snip20_contract_store, staking_contract_store, create_token_pair, convert_to_contract_link};
    ...
    use shadeswap_shared::core::{ContractLink, ContractInstantiationInfo, TokenPair, TokenPairAmount};
    ...
    println!("chain_id {}",  router.block_info().chain_id);
    ...
    let token_0 = generate_snip20_contract(
        &mut router, 
        "ETH".to_string(), 
        "ETH".to_string(), 
        18
    ...
    let token_1 = generate_snip20_contract(
        &mut router, 
        "USDT".to_string(), 
        "USDT".to_string(), 
        18
    ...
    let pair = create_token_pair(
        &convert_to_contract_link(&token_0),
        &convert_to_contract_link(&token_1)
    );
    let add_liqudity = ExecuteMsg::AddLiquidityToAMMContract { 
        deposit: TokenPairAmount{
            pair: pair.clone(),
            amount_0: Uint128::new(10000u128),
            amount_1: Uint128::new(10000u128),
        }, 
        expected_return: Some(Uint128::new(1000)), 
        staking: Some(false) 
    };
