...
use network_integration::cli_commands::amm_pair_lib::add_amm_pairs_no_staking;
use network_integration::cli_commands::amm_pair_lib::add_amm_pairs_with_staking;
...
fn run_testnet() -> Result<()> {
    ...
    let pair_contract_code_hash = store_and_return_contract(AMM_PAIR_FILE, ACCOUNT_KEY, Some(STORE_GAS), Some("test"))?.code_hash;
    ...
    let factory_contract =
        create_factory_contract(ACCOUNT_KEY, "test", &mut reports, &admin_contract.address)
            .unwrap();
    ...
    let token_pair_1 = TokenPair(
        token_1.clone(),
        token_2.clone(),
    );

    let token_pair_2 = TokenPair(
        token_native.clone(),
        token_1.clone(),
    );

    add_amm_pairs_with_staking(
    ...
    s_sCRT.address.clone(), s_sSHD.address.clone(), snip_20_code_hash.clone(),
