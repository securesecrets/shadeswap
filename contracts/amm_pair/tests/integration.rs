...
use shadeswap_shared::msg::amm_pair::{ExecuteMsg, InitMsg, QueryMsg, QueryMsgResponse};

#[cfg(not(target_arch = "wasm32"))]
#[test]
pub fn amm_pair_integration_tests_with_custom_token() {
    use amm_pair::contract::{execute, instantiate, query};
    use cosmwasm_std::{Coin, Timestamp, Uint128};
    use multi_test::admin::admin_help::init_admin_contract;
    use multi_test::help_lib::integration_help_lib::{
        convert_to_contract_link, create_token_pair, get_amm_pair_config,
        get_pair_liquidity_pool_balance, get_snip20_balance, increase_allowance,
        mint_deposit_snip20, roll_blockchain, send_snip20_with_msg, set_viewing_key,
        store_init_factory_contract,
    };
    use multi_test::util_addr::util_addr::OWNER;
    use shadeswap_shared::core::{
        ContractInstantiationInfo, CustomFee, Fee, TokenAmount, TokenPairAmount,
    };
    ...
    pub fn staking_contract_store() -> Box<dyn Contract<Empty>> {
        let contract =
            ContractWrapper::new_with_empty(staking_execute, staking_instantiate, staking_query);
        Box::new(contract)
    }
    ...
    let token_0_contract =
        generate_snip20_contract(&mut router, "ETH".to_string(), "ETH".to_string(), 18).unwrap();
    let token_1_contract =
        generate_snip20_contract(&mut router, "USDT".to_string(), "USDT".to_string(), 18).unwrap();
    let token_2_contract =
        generate_snip20_contract(&mut router, "USDT".to_string(), "USDT".to_string(), 18).unwrap();
    ...
    let factory_contract_info =
        store_init_factory_contract(&mut router, &convert_to_contract_link(&admin_contract))
            .unwrap();
    ...
    let query: QueryMsgResponse = router
        .query_test(
    ...
    match query {
        QueryMsgResponse::GetConfig {
            ...
            assert_ne!(
                staking_contract.unwrap().address.to_string(),
                "".to_string()
            );
        }
    ...
    increase_allowance(
        &mut router,
        &token_0_contract,
        Uint128::new(10000000000000u128),
        &amm_pair_contract.address,
        &owner_addr,
    )
    .unwrap();
    roll_blockchain(&mut router, 1).unwrap();
    ...
    match query {
        QueryMsgResponse::GetConfig {
            ...
            let balance = get_snip20_balance(
                &mut router,
                &ContractInfo {
                    address: lp_token.address.clone(),
                    code_hash: lp_token.code_hash.to_string(),
...
pub fn amm_pair_integration_tests_native_token() {
    ...
    use multi_test::admin::admin_help::init_admin_contract;
    use multi_test::help_lib::integration_help_lib::{
        convert_to_contract_link, create_token_pair_with_native, get_amm_pair_config,
        get_pair_liquidity_pool_balance, get_snip20_balance, increase_allowance,
        mint_deposit_snip20, roll_blockchain, send_snip20_with_msg, set_viewing_key,
        store_init_factory_contract,
    };
...
pub fn test_sslp_with_two_virtual_providers() {
    ...
    use amm_pair::contract::{execute, instantiate, query};
    use cosmwasm_std::{from_binary, Coin, Response, StdError, Timestamp, Uint128};
    use multi_test::admin::admin_help::init_admin_contract;
    use multi_test::help_lib::integration_help_lib::{
        convert_to_contract_link, create_token_pair, create_token_pair_with_native,
        get_amm_pair_config, get_pair_liquidity_pool_balance, get_snip20_balance,
        increase_allowance, mint_deposit_snip20, roll_blockchain, send_snip20_with_msg,
        set_viewing_key, store_init_factory_contract,
    };
    ...
    assert!(
        total_liquidity.1 < Uint128::new(100000200u128)
            && total_liquidity.1 > Uint128::new(100000000u128)
    );
    ...
    let remove_msg = to_binary(&InvokeMsg::RemoveLiquidity {
        from: Some(owner_addr.to_string()),
        single_sided_withdraw_type: Some(TokenType::NativeToken {
            denom: "uscrt".to_string(),
        }),
        single_sided_expected_return: None,
    })
