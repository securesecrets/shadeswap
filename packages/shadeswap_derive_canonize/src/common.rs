    ...
    match &strukt.fields {
        Fields::Named(fields) => {
            ...
            for field in fields.named.iter() {
                ...
                let value = if canonize {
                    parse_quote!(#name: Canonize::canonize(self.#name, api)?)
                } else {
                    parse_quote!(#name: Humanize::humanize(self.#name, api)?)
                };
        ...
        Fields::Unnamed(fields) => {
            ...
            for i in 0..fields.unnamed.len() {
                ...
                let value = if canonize {
                    parse_quote!(Canonize::canonize(self.#lit, api)?)
                } else {
                    parse_quote!(Humanize::humanize(self.#lit, api)?)
                };
