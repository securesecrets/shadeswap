...
use network_integration::utils::{
    generate_label, init_snip20,init_contract_factory,  print_contract, print_header, print_vec, print_warning,
    AMM_PAIR_FILE, FACTORY_FILE, GAS, LPTOKEN20_FILE, ROUTER_FILE, SHADE_DAO_KEY, 
    STAKING_FILE, VIEW_KEY, SNIP20_FILE, InitConfig,
};
...
use shadeswap_shared::{
    ...
    msg::{
        ...
        factory::{
            HandleMsg as FactoryHandleMsg, QueryMsg as Factory, QueryMsg as FactoryQueryMsg,
            QueryResponse as FactoryQueryResponse, InitMsg as FactoryInitMsg
        },
        router::{
            HandleMsg as RouterHandleMsg, InitMsg  as RouterInitMsg,            
        },
    },
...
fn main() -> io::Result<()> {
    ...
    {      
        ...
        if input == 5 {
            ...
            //         HumanAddr::from(recipient.clone().trim()),
            //         Uint128(1000000),
            //         "1000000uscrt",
            //         &mut reports,
            //         btc_contrat.clone())?;       
...
fn create_new_deployment(account_name: &str, backend: &str) -> io::Result<()> 
{
    ...
    let pairs = TokenPair::<HumanAddr>(
        TokenType::CustomToken {
            contract_addr: contract_0.address.clone().into(),
            token_code_hash: contract_0.code_hash.to_string(),
        },
        TokenType::CustomToken {
            contract_addr: contract_1.address.clone().into(),
    ...
    increase_allowance(pair_address.clone(),Uint128(100000000), contract_0.clone(),
        account_name, backend, &mut reports)?;
    increase_allowance(pair_address.clone(),Uint128(100000000), contract_1.clone(),
        account_name, backend, &mut reports)?;

    mint_snip20(account_name, backend, HumanAddr::from(""),
                Uint128(100000), "100000uscrt",&mut reports, contract_0.clone())?;

    mint_snip20(account_name, backend, HumanAddr::from(""),
                Uint128(100000), "100000uscrt",&mut reports, contract_1.clone())?;
...
{   
    ...
    reports, 10, account_name, backend)?;
...
fn init_snip20_contract(symbol: &str, name: &str, reports: &mut Vec<Report>, 
    ...
    let s_contract = init_snip20(
        "ETH".to_string(), //name.to_string(),
        "ETH".to_string(), //symbol.to_string(),
        8, //decimal,
        Some(config),
        reports,
        &"mike".to_string(), //&account_name,
        Some(&SNIP20_FILE.replace("../", "")),
        &"test"//&keyring_backend
    )?.1;
...
fn create_amm_pair_and_set_liquidity(
    ...
    increase_allowance(pair_address.clone(),Uint128(100000000), contract_token_0.clone(),
        account_name, backend, reports)?;
    increase_allowance(pair_address.clone(),Uint128(100000000), contract_token_1.clone(),
        account_name, backend, reports)?;

    mint_snip20(account_name, backend, HumanAddr::from("secret10x73565sfrn8veawzz69le0na4y5nluqlwdvqu"),
                Uint128(100000000), "100000000uscrt", reports, contract_token_0.clone())?;

    mint_snip20(account_name, backend, HumanAddr::from("secret10x73565sfrn8veawzz69le0na4y5nluqlwdvqu"),
                Uint128(100000000), "100000000uscrt", reports, contract_token_1.clone())?;
...
-> io::Result<NetContract>
{
    ...
    let factory_contract = init_contract_factory(
        &account_name, 
        &backend,
        &FACTORY_FILE.replace("../", ""), &init_msg, reports)?;
   
    println!("Factory address {}", factory_contract.address.clone().to_string());
    Ok(factory_contract)
}

fn add_amm_pairs(pairs: TokenPair<HumanAddr>,
    staking_contract: StoredContract,
    factory_contract: NetContract,
    backend: &str,
    account_name: &str,
...
pub fn get_token_type(pairs: TokenPair<HumanAddr>) -> io::Result<(String,String)>{
...
pub fn init_router_contract(factory_contract: NetContract,
account_name: &str,
backend: &str,
...
) -> io::Result<()>
{
    handle(
        &snip20_reference_impl::msg::HandleMsg::IncreaseAllowance {
            spender: HumanAddr(String::from(pair_address)),
...
pub fn mint_snip20(
    ...
    let msg = snip20_reference_impl::msg::HandleMsg::Mint { padding: None, recipient: recipient, amount: amount, memo: None };
    handle(
        &msg,
        &contract_snip20,
        account_name,
