...
use crate::contract::{instantiate, {create_viewing_key, calculate_price, calculate_swap_result,swap, query, add_liquidity, handle}};
use std::hash::Hash;
use cosmwasm_std::{BankQuery, AllBalanceResponse, to_vec,log,  Coin, StdResult, BalanceResponse, StdError, QueryRequest, Empty, Uint128, to_binary, QuerierResult, from_slice, Querier, testing::{MockApi, MockStorage}, Extern, ContractInfo, MessageInfo, BlockInfo, Env, Api, Storage, WasmQuery};
...
pub mod tests {
    ...
    fn assert_init_config() -> StdResult<()> {       
        ...
        env.contract.address = Addr("ContractAddress".to_string());
        ...
        let msg = InitMsg {
            ...
            factory_info: ContractLink {
                address: Addr(String::unchecked("FACTORYADDR")),
        ...
        assert!(init(deps.as_mut(), env.clone(), msg).is_ok());
      
 
        let test_view_key = create_viewing_key(&env,seed.clone(),entropy.clone());
        // load config
        let config = config_r(deps.as_mut().storage).unwrap();
    ...
    fn assert_store_and_load_config_success() -> StdResult<()>{
        let deps = mock_dependencies();
        let token_pair = mk_token_pair();
        let config = make_init_config(deps.as_mut(),token_pair)?;   
        store_config(&mut deps, config.clone())?;
        let stored_config = load_config(deps)?;
    ...
    fn assert_add_address_to_whitelist_success()-> StdResult<()>{
        let mut deps = mock_dependencies();
        let env = mkenv();       
        let addressA = "TESTA".to_string();
        let addressB = "TESTB".to_string();
        let addressC = "TESTC".to_string();
    ...
    fn assert_remove_address_from_whitelist_success()-> StdResult<()>{
        ...
        let address_b = "TESTB".to_string();
        let address_c = "TESTC".to_string();
    ...
    fn assert_load_address_from_whitelist_success()-> StdResult<()>{
        ...
        let is_addr = is_address_in_whitelist(&mut deps.storage, Addr("TESTD".to_string()).clone())?;  
    ...
    fn assert_swap_native_snip20()-> StdResult<()>{
        ...
        let address_a = Addr("TESTA".to_string());      
...
fn make_init_config<S: Storage, A: Api, Q: Querier>(
    deps: DepsMut, 
    token_pair: TokenPair<Addr>) -> StdResult<Config> {    
    let seed = to_binary(&"SEED".to_string())?;
    let entropy = to_binary(&"ENTROPY".to_string())?;
    let env = mkenv();  
    ...
    let msg = InitMsg {
        ...
        factory_info: ContractLink {
            address: Addr(String::unchecked(FACTORY_CONTRACT_ADDRESS)),
...
fn mk_token_pair() -> TokenPair<Addr>{
    let pair =  TokenPair(
        TokenType::CustomToken {
            contract_addr: Addr(CUSTOM_TOKEN_1.to_string().clone()),
            token_code_hash: CUSTOM_TOKEN_1.to_string()
        },            
        TokenType::CustomToken {
            contract_addr: Addr(CUSTOM_TOKEN_2.to_string().clone()),
...
fn mk_native_token_pair() -> TokenPair<Addr>{
    let pair =  TokenPair(
        TokenType::CustomToken {
            contract_addr: Addr(CUSTOM_TOKEN_2.to_string()),
            token_code_hash: CUSTOM_TOKEN_2.to_string()
        },            
...
fn mk_custom_token_amount(amount: Uint128, token_pair: TokenPair<Addr>) -> TokenAmount<Addr>{    
...
fn mk_custom_token(address: String) -> TokenType<Addr>{
    TokenType::CustomToken {
        contract_addr: Addr(address.clone()),
        token_code_hash: "TOKEN0_HASH".to_string()
    }
}

fn mk_native_token() -> TokenType<Addr>{
    TokenType::NativeToken{
        denom: "uscrt".to_string()
    }
}

fn mk_amm_settings() -> AMMSettings{
    AMMSettings{
        lp_fee: Fee{
            nom: 2,
            denom: 100
...
fn mock_config(env: Env) -> StdResult<Config>
{    
    let seed = to_binary(&"SEED".to_string())?;
    let entropy = to_binary(&"ENTROPY".to_string())?;
    ...
    Ok(Config {       
        ...
        pair:      mk_token_pair(),
        staking_contract: Addr::unchecked(MOCK_CONTRACT_ADDR),
...
impl Querier for MockQuerier {
                        ...
                        match contract_addr.as_str() {
                            FACTORY_CONTRACT_ADDRESS => {
                                let amm_settings = shadeswap_shared::amm_pair::AMMSettings {
                                    ...
                                    shade_dao_address: ContractLink {
                                        address: Addr(String::unchecked("DAO")),
                                        code_hash: "".to_string(),
                                    }
                                };
    ...
    fn query_balance<U: Into<Addr>>(&self, address: U, denom: &str) -> StdResult<Coin> {
    ...
    fn query_all_balances<U: Into<Addr>>(&self, address: U) -> StdResult<Vec<Coin>> {
...
pub mod tests_calculation_price_and_fee{
    ...
    use serde::Deserialize;
    use serde::Serialize;
    use crate::test::help_test_lib::{{ mk_amm_settings_a, mkenv, 
        ...
        mk_native_token_pair_test_calculation_price_fee}};
    ...
    {     
        ...
        let swap_result = calculate_swap_result(&deps.querier, &amm_settings, &config, 
            ...
            &mut deps.storage, Addr("Test".to_string().clone()), Some(true));
    ...
    {     
        ...
        let swap_result = calculate_swap_result(&deps.querier, &amm_settings, &config, 
            &mk_custom_token_amount_test_calculation_price_fee(Uint128::from(offer_amount), config.pair.clone()), 
            &mut deps.storage, Addr("Test".to_string().clone()), None);
        assert_eq!(Uint128::from(expected_amount), swap_result?.result.return_amount);
        Ok(())
    }
    ...
    fn assert_slippage_add_liqudity_with_right_ration_success() -> StdResult<()>{
        ...
        let token_pair = mk_token_pair_test_calculation_price_fee();
        let env = mkenv();
        let config = make_init_config_test_calculate_price_fee(deps, token_pair.clone(), None)?;         
        // set lp token contract for liqudity
        let mut config = load_config(&deps)?;
        config.lp_token_info = ContractLink{ 
            address: Addr::unchecked(LP_TOKEN_ADDRESS),
            code_hash: "CODE_HASH".to_string()
        };
        store_config(&mut deps, config.clone())?;
        let offer_amount: u128 = 2000;          
...
pub mod help_test_lib {
    ...
    use shadeswap_shared::amm_pair::{{AMMPair, AMMSettings}};
    use crate::state::{amm_pair_storage::{{ store_config, load_config,
        remove_whitelist_address,is_address_in_whitelist, add_whitelist_address,load_whitelist_address, }}, config_r};
                ...
                QueryRequest::Wasm(msg) => {
                    match msg {
                        WasmQuery::Smart { contract_addr, .. } => {
                            ...
                            match contract_addr.as_str() {                          
                                CUSTOM_TOKEN_2 => {   
                                    let result = cosmwasm_std::SystemResult::Ok(cosmwasm_std::ContractResult::Ok(
                    ...
                    match msg {
                        WasmQuery::Smart { contract_addr, .. } => {
                            ...
                            match contract_addr.as_str() {                          
                                ...
                                FACTORY_CONTRACT_ADDRESS => {
                                    let result = cosmwasm_std::SystemResult::Ok(cosmwasm_std::ContractResult::Ok(
                                        to_binary(&FactoryQueryResponse::GetAMMSettings{
                                ...
                                P_TOKEN_ADDRESS => {
                                    let result = cosmwasm_std::SystemResult::Ok(cosmwasm_std::ContractResult::Ok(
                                        to_binary(&TokenInfoResponseMock{
    ...
    pub fn mk_custom_token_amount_test_calculation_price_fee(amount: Uint128, token_pair: TokenPair<Addr>) -> TokenAmount<Addr>{    
        let token = TokenAmount{
            token: token_pair.0.clone(),
            amount: amount.clone(),
        };
    ...
    pub fn mk_native_token_pair_test_calculation_price_fee() -> TokenPair<Addr>{
        let pair =  TokenPair(
            TokenType::CustomToken {
                contract_addr: Addr(CUSTOM_TOKEN_2.to_string()),
                token_code_hash: CUSTOM_TOKEN_2.to_string()
    ...
    // pub fn mock_config_test_calculation_price_fee(env: Env) -> StdResult<Config<Addr>> {
    // {    
    //     let seed = to_binary(&"SEED".to_string())?;
    //     let entropy = to_binary(&"ENTROPY".to_string())?;
    ...
    -> StdResult<Config> {    
        ...
        assert!(instantiate(deps, env.clone(),mock_info, msg).is_ok());
        let config = config_r(deps)?;    // set staking contract
    ...
    pub fn mk_token_pair_test_calculation_price_fee() -> TokenPair<Addr>{
        let pair =  TokenPair(
            TokenType::CustomToken {
                contract_addr: Addr(CUSTOM_TOKEN_1.to_string().clone()),
                token_code_hash: CUSTOM_TOKEN_1.to_string()
